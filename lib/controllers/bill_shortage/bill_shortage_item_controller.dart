...
import '../../data/repo/bill_shortage/bill_shortage_item_repo.dart';
import '../../libraries.dart';

abstract class BillShortageItemController extends GetxController {
  addBillShortageItem({
...
class BillShortageItemControllerImp extends BillShortageItemController {
  var billShortageItemsList = <Item>[].obs;
  List<Item> billShortageSearchItemsList = <Item>[].obs;
  bool isShown = true;
  var isLoading = false.obs;
  List<Item> billShortageAllItemsInRange = <Item>[].obs;
  List<Item> billShortageItemInRange = <Item>[].obs;
  var billShortageItemsInDay = <Item>[].obs;
  ...
  addBillShortageItem({
    ...
    var addBillShortage = await BillShortageItemRepo.addBillShortageItem(
        item: item,
  ...
  editItems(
    ...
    isLoading.value = true;
    var editItem = await BillShortageItemRepo.editBillShortageItem(
  ...
  deleteAllBillItems({required String billId}) async {
    isLoading(true);
    var item = await BillShortageItemRepo.deleteBillShortageItem(id: billId);
    if (item.status == 'suc') {
      isLoading(false);
      update();
  ...
  getitemsbyBillService({required String billId}) async {
    var items = await BillShortageItemRepo.getBillShortageItems(billId: billId);
    return items.data;
  }
  ...
  getItemsbyBillId({required String billId}) async {
    ...
    isLoading(true);
    billShortageItemsList.value = items;
    isLoading(false);
    if (items.isEmpty) {
      isLoading(false);
      update();
  ...
  getAllItemsinRange({
    ...
    isLoading.value = true;
    var items = await BillShortageItemRepo.getBillShortageAllItemsInRange(
      starDate: start,
      endDate: end,
    );
    try {
      if (items.status == "suc") {
        billShortageAllItemsInRange.addAll(items.data);
        isLoading(false);
        update();
  ...
  getIteminRange({
    ...
    isLoading.value = true;
    var item = await BillShortageItemRepo.getBillShortageItemInRange(
      starDate: start,
      endDate: end,
      itemNum: itemNum,
    );
    try {
      if (item.status == "suc") {
        billShortageItemInRange.addAll(item.data);
        isLoading(false);
        update();
      } else if (item.status == "fail") {
        isLoading(false);
  ...
  getAllItemsInDay({required DateTime date}) async {
    ...
    update();
    try {
      var items = await BillShortageItemRepo.getAllItemsInDay(
          date: formatter.format(date));
      if (items.status == 'suc') {
        billShortageItemsInDay.addAll(items.data);
        isLoading(false);
        update();
      } else {
        isLoading(false);
