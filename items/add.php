...
$item_wight     = filterRequest('item_wight');            //15
$stmt = $con->prepare("SELECT * FROM `items` WHERE `item_num` =?");
$stmt->execute(array($item_num));
$cont = $stmt->rowCount();
if($cont > 0){
    echo json_encode(array('status' => 'found'));
}else{
    $stmt = $con->prepare("INSERT INTO `items`(`item_name`, `item_num`, `item_sup`, `item_price_in`, `item_price_out`, `item_count`, `bill_id`,`item_quant`, `item_min` , `item_max`, `item_pakage`,`item_piec`, `ingredients_number`, `kind`, `item_wight`)  VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
    $stmt->execute(array($item_name, $item_num, $item_sup, $item_price_in,  $item_price_out, $item_count, $bill_id, $item_quant, $item_min, $item_max, $item_pakage, $item_piec, $ingredients_number, $kind, $item_wight));
    $cont = $stmt->rowCount();
    if($cont > 0){
